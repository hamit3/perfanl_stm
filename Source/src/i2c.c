...
void I2C_Write_Bulk(uint32_t Device, uint32_t Register, uint32_t *Data, uint32_t Lenght)
{
	...
	for(i=0; i<Lenght; i++)      // TIMEOUT KOY, ERROR DURUMDA ÇIKIS KOY, WATCHDOG EKLE VSVS
	{
		//Check Tx empty before writing to it
		if((I2C1->ISR & I2C_ISR_TXE) == (I2C_ISR_TXE))
		{
...
void I2C_Write_NoAck(uint32_t Device, uint32_t Register, uint32_t *Data, uint32_t Lenght)
{
	int i = 0;
	//Reset CR2 Register
	I2C1->CR2 = 0x00000000;
